    ...
    <div className="bg-bg text-ink">
      <section className="mx-auto max-w-[1440px] px-5 pb-16 pt-14 md:px-10 md:pb-24 md:pt-20">
        <div className="grid gap-10 lg:grid-cols-[1.15fr_.85fr] lg:items-end">
          <div>
            ...
            <h1 className="max-w-6xl font-display text-[clamp(5rem,14vw,12rem)] font-black leading-[.76] tracking-[-.075em]">
              hi, i’m<br /><span className="text-blue">dominic.</span>
            </h1>
          </div>
      ...
      <section className="mx-auto max-w-[1440px] px-5 py-20 md:px-10 md:py-28">
        <div className="grid gap-10 lg:grid-cols-[.7fr_1.3fr]">
          ...
          <div className="max-w-4xl">
            <h2 className="font-display text-4xl font-black leading-[.95] md:text-7xl">I couldn’t see myself anywhere in the world. So I started creating.</h2>
            <div className="mt-8 grid gap-6 text-lg leading-relaxed md:grid-cols-2">
              <p>I wanted to meet people like me, have more positive conversations, and build a life that felt happy. At first, that meant fashion-forward videos, favorite music, and experimenting with edits.</p>
              <p>Then I moved to Los Angeles in November 2024. My creativity opened up, the support got louder, and creating stopped feeling like something I was trying — it became something I was building.</p>
            </div>
            <Link href="/about" className="mt-8 inline-flex font-bold text-blue hover:underline">Read my story →</Link>
          </div>
        </div>
      </section>

      <section className="border-y border-black/15 bg-surface">
        <div className="mx-auto grid max-w-[1440px] grid-cols-2 md:grid-cols-4">
          {stats.map(([value, label], i) => <div key={label} className={`border-black/15 px-5 py-9 ${i < 3 ? "border-r" : ""} md:px-8 md:py-12`}><p className="font-display text-4xl font-black tracking-[-.05em] md:text-6xl">{value}</p><p className="mt-2 max-w-[170px] text-xs font-bold uppercase tracking-[.12em] text-ink-muted">{label}</p></div>)}
        </div>
      </section>
      ...
      <section className="mx-auto max-w-[1440px] px-5 py-20 md:px-10 md:py-28">
        ...
        <div className="grid gap-5 lg:grid-cols-3">
          {featured.map((item, i) => <Link href={item.href} key={item.title} className={`work-card ${item.tone} ${i === 1 ? "lg:mt-16" : i === 2 ? "lg:mt-8" : ""} p-7 md:p-9`}><div className="flex items-start justify-between gap-4"><span className="text-[10px] font-extrabold uppercase tracking-[.16em] opacity-70">{item.type}</span><span className="arrow text-2xl">↗</span></div><div className="mt-28 md:mt-40"><h3 className="font-display text-3xl font-black leading-[.95] md:text-4xl">{item.title}</h3><p className="mt-4 max-w-md text-sm leading-relaxed opacity-80">{item.body}</p></div></Link>)}
        </div>
      </section>

      <section className="bg-red px-5 py-20 md:px-10 md:py-28">
        <div className="mx-auto max-w-[1440px]">
          <p className="mb-5 text-[10px] font-extrabold uppercase tracking-[.16em]">03 · The bigger picture</p>
          <div className="grid gap-10 lg:grid-cols-[1.25fr_.75fr] lg:items-end">
            <h2 className="font-display text-5xl font-black leading-[.86] tracking-[-.06em] md:text-8xl">I want trans men to see themselves — and share more of our own stories.</h2>
            <div><p className="text-lg leading-relaxed">Transmasculine people are rarely at the forefront of media productions. I’m interested in closing that gap through videos, collaborations, experiences, and brands that make people feel included and powerful.</p><Link href="/t-time-media" className="mt-8 inline-flex btn-ghost bg-bg">See T-Time Media →</Link></div>
          </div>
        </div>
      </section>
